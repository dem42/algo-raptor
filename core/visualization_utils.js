var ALGORITHM_MODULE = (function(ALGORITHM_MODULE, d3, $) {
    ...
    _my.vislib.animatePath = function(path, duration, delay, make_proportional) {
    ...
    _my.vislib.animateGrowingArrow = function(svg, path, duration, delay, make_proportional) {
	var arrow = svg.append("svg:path")
	    .attr("d", d3.svg.symbol().type("triangle-down")(10,1));

	_my.vislib.animatePath(path, duration, delay, make_proportional);
	_my.vislib.animateMovingAlongAPath(arrow, path, duration, delay, make_proportional, true, -90);
    }
     ...
     * maybe in the future
     */
    _my.vislib.animateMovingAlongAPath = function(movable_selection, path, duration, delay, make_proportional, with_rotate, with_rotate_extra_angle) {
	if (make_proportional !== undefined && make_proportional === true) {
	    // make duration proportional to totalLength to create a smoother animation
	    duration = duration * totalLength;
	}
	...
	function lineData(d){
	    ...
	    lineData.target = function(d) { return d.targer; }
