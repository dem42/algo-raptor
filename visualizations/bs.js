ALGORITHM_MODULE.bsearch_module = (function chart(ALGORITHM_MODULE, $, d3, bootbox) {
    ...
    layout_def.introductionParagraph.html("<p>The lesser known version of binary search is the deferred evaluation version. The advantage of this version is that it finds the first index where our condition holds! This is important and often super useful because in classical binary search the returned index is any index that matches our condition. The disadvantage is that we never terminate sooner than in <var>log N</var> steps</p><p>In the deferred evaluation version the equality-to-the-key check is not performed inside the loop. Instead we just check whether the value at the current position is smaller than the key.</p>");
    ...
    layout_norm.introductionParagraph.html("Binary search is the big kahuna. The alpha and omega. The first algorithm for most people but also sometimes one of the trickiest to get right in programming competitions. The idea is simple enough; if you have a sorted sequence you can find an element in only <var>log N</var> steps by jumping to the middle of the sequence, checking the value there against the element you are searching for and then repeating the process on either the top or lower half of the sequence.");
